...
import { loopTimerCommand } from "@/cli/cmd/run/loop/loop-timer"
...
it("/loop timer pause/resume 切换 overlay", () =>
  Effect.gen(function* () {
    const out = yield* loopTimerCommand("pause")
    expect(out).toContain("paused")
    const paused = yield* TimerDaemon.isPaused
    expect(paused).toBe(true)
    yield* loopTimerCommand("resume")
...
it("/loop timer show 显示 daemon 配置", () =>
  Effect.gen(function* () {
    const out = yield* loopTimerCommand("")
