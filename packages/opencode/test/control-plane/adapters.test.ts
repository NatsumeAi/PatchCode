...
describe("control-plane/adapters", () => {
  test("isolates custom adapters by project", async () => {
    ...
    const one = Project.ID.make(`project-${Math.random().toString(36).slice(2)}`)
    const two = Project.ID.make(`project-${Math.random().toString(36).slice(2)}`)
  ...
  test("latest install wins within a project", async () => {
    const type = `demo-${Math.random().toString(36).slice(2)}`
    const id = Project.ID.make(`project-${Math.random().toString(36).slice(2)}`)
