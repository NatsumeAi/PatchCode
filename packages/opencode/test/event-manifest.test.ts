...
describe("public event manifest", () => {
  test("contains every latest public wire type once", () => {
    ...
    expect(EventManifest.Latest.has("session.next.subagent.started")).toBe(true)
    expect(EventManifest.Latest.has("session.next.subagent.heartbeat_lost")).toBe(true)
    expect(EventManifest.Durable.get("session.next.subagent.completed.1")).toBe(SessionEvent.Subagent.Completed)
