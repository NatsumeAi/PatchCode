...
export interface LineCommentProps extends ComponentProps<"div"> {
  ...
  /** Typically an overflow menu trigger; use `LineCommentOverflowIcon` inside `line-comment-kit-overflow`. */
  actions?: JSX.Element
}
...
export function LineComment(props: LineCommentProps) {
  ...
  return (
    ...
    >
      <div data-slot="line-comment-kit-shell">
        <div data-slot="line-comment-kit-column">
          <div data-slot="line-comment-kit-text">{local.comment}</div>
          <div data-slot="line-comment-kit-meta">{local.selection}</div>
        </div>
        <Show when={local.actions}>{(actions) => <div data-slot="line-comment-kit-tools">{actions()}</div>}</Show>
      </div>
    </div>
  )
}
...
export function LineCommentEditor(props: LineCommentEditorProps) {
  ...
  return (
    ...
    >
      <div data-slot="line-comment-kit-shell">
        <div data-slot="line-comment-kit-field">
          <div data-slot="line-comment-kit-label">{heading()}</div>
          <textarea
            ref={(el) => {
              textareaRef = el
            }}
            data-slot="line-comment-kit-textarea"
            rows={local.rows ?? 3}
          ...
          <Show when={mentionOpen() && mention.flat().length > 0}>
            <div data-slot="line-comment-kit-mention-list">
              <For each={mention.flat().slice(0, 10)}>
                {(item) => {
                  ...
                  return (
                    <button
                      type="button"
                      data-slot="line-comment-kit-mention-item"
                    ...
                    >
                      ...
                      <div data-slot="line-comment-kit-mention-path">
                        <span data-slot="line-comment-kit-mention-dir">{directory}</span>
                        <Show when={name}>
                          <span data-slot="line-comment-kit-mention-file">{name}</span>
                        </Show>
                      </div>
                    </button>
                  )
        ...
        <div data-slot="line-comment-kit-footer">
          <div data-slot="line-comment-kit-footer-meta">{local.selection}</div>
          <div data-slot="line-comment-kit-footer-actions">
