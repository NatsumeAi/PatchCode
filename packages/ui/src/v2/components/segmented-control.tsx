...
export function SegmentedControl(props: SegmentedControlProps) {
  ...
  const focusNext = (from: HTMLButtonElement, direction: 1 | -1) => {
    ...
    const buttons = Array.from(
      root.querySelectorAll<HTMLButtonElement>(`button[data-slot="segmented-control-kit-item"]`),
    ).filter((b) => !b.disabled)
...
export function SegmentedControlItem(props: SegmentedControlItemProps) {
  ...
  const onKeyDown: JSX.EventHandlerUnion<HTMLButtonElement, KeyboardEvent> = (e) => {
    ...
    // accessibility stuff
      ...
      const first = root?.querySelector<HTMLButtonElement>(
        `button[data-slot="segmented-control-kit-item"]:not(:disabled)`,
      )
      first?.focus()
    } else if (e.key === "End") {
      e.preventDefault()
  ...
  return (
    <button
      {...rest}
      type="button"
      data-slot="segmented-control-kit-item"
      ...
      onKeyDown={onKeyDown}
    >
      <span data-slot="segmented-control-kit-item-label">{local.children}</span>
    </button>
  )
}
