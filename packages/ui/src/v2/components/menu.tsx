...
const ChevronRight: Component = () => (
  <svg
    data-slot="menu-kit-item-chevron"
    width="16"
    height="16"
    viewBox="0 0 16 16"
    fill="none"
...
function ItemBody(
  ...
  return (
    <>
      <span data-slot="menu-kit-item-content">{props.children}</span>
      <Show when={props.shortcut}>{(shortcut) => <span data-slot="menu-kit-item-shortcut">{shortcut()}</span>}</Show>
      <Show when={props.badge}>{(badge) => <span data-slot="menu-kit-item-badge">{badge()}</span>}</Show>
      {props.trailing}
    </>
  )
}
...
function MenuItem(props: ParentProps<MenuItemProps>) {
  ...
  return (
    <DropdownMenu.Item {...r} data-component="menu-kit-item" classList={{ ...s.classList, [s.class ?? ""]: !!s.class }}>
  ...
  return (
    ...
    >
      <ItemBody
        ...
        badge={s.badge}
        trailing={
          <DropdownMenu.ItemIndicator data-slot="menu-kit-item-indicator" forceMount>
            <CheckMark />
...
function MenuSubContent(props: ComponentProps<typeof DropdownMenu.SubContent>) {
  ...
  return (
    <DropdownMenu.SubContent
      {...r}
      data-component="menu-kit-content"
      classList={{ ...s.classList, [s.class ?? ""]: !!s.class }}
    />
  )
}
...
function MenuGroupLabel(props: ComponentProps<typeof DropdownMenu.GroupLabel>) {
  ...
  return (
    <DropdownMenu.GroupLabel
      {...r}
      data-slot="menu-kit-group-label"
      classList={{ ...s.classList, [s.class ?? ""]: !!s.class }}
    />
  )
}
