...
import type { FileContent, SnapshotFileDiff, VcsFileDiff } from "@opencode-ai/sdk/api"
...
import type { SessionReviewExpandMode } from "./session-review"
import { createLineCommentController } from "./line-comment-annotations"
import { shouldVirtualizeReviewDiff } from "./session-review-file-preview-virtualize"
...
export type SessionReviewFilePreviewProps = {
  file: string
  diff: ReviewDiff
    ...
    <div onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
      <Menu gutter={4}>
        <Menu.Trigger
          as="button"
          type="button"
          data-slot="line-comment-kit-overflow"
...
export function SessionReviewFilePreview(props: SessionReviewFilePreviewProps) {
  const i18n = useI18n()
  ...
  return (
    <>
      <div data-slot="session-review-kit-file-header">
        <div data-slot="session-review-kit-file-title">
          <div data-slot="session-review-kit-file-status" data-type={statusType(view().status)}>
            {statusLabel(view().status)}
          </div>
          <FileIcon node={{ path: props.file, type: "file" }} />
          <span data-slot="session-review-kit-file-name">{getFilename(props.file)}</span>
          <Show when={props.file.includes("/")}>
            <span data-slot="session-review-kit-file-path">{getDirectory(props.file)}</span>
          </Show>
        </div>
        <div data-slot="session-review-kit-file-diff">
      ...
      <div
        ...
        data-slot="session-review-kit-diff-scroll"
      >
        <Show
          when={diffCanRender() || mediaKind()}
          fallback={
            <div data-slot="session-review-kit-empty">
