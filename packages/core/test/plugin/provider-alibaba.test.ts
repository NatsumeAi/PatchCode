...
describe("AlibabaPlugin", () => {
    ...
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("alibaba"), Model.ID.make("qwen")),
          api: { id: Model.ID.make("qwen"), type: "aisdk", package: "test-provider" },
        }),
  ...
  it.effect("matches the old bundled Alibaba SDK provider naming", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("custom-alibaba"), Model.ID.make("qwen")),
          api: { id: Model.ID.make("qwen"), type: "aisdk", package: "test-provider" },
        }),
        package: "@ai-sdk/alibaba",
  ...
  it.effect("uses the old default languageModel(api.id) behavior", () =>
    Effect.gen(function* () {
      ...
      const aisdk = yield* AISDK.Service
      yield* addPlugin()
      const item = Model.Info.make({
        ...Model.Info.empty(Provider.ID.make("alibaba"), Model.ID.make("alias")),
        api: { id: Model.ID.make("qwen-plus"), type: "aisdk", package: "test-provider" },
      })
