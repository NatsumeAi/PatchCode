...
describe("GitLabPlugin", () => {
        ...
        Effect.gen(function* () {
          ...
          yield* aisdk.runSDK({
            model: Model.Info.make({
              ...Model.Info.empty(Provider.ID.make("gitlab"), Model.ID.make("claude")),
              api: { id: Model.ID.make("claude"), type: "aisdk", package: "test-provider" },
            }),
  ...
  it.effect("uses workflowChat for duo workflow models and preserves selectedModelRef", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runLanguage({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("gitlab"), Model.ID.make("duo-workflow-custom")),
          api: { id: Model.ID.make("duo-workflow-custom"), type: "aisdk", package: "test-provider" },
          request: {
            headers: {},
  ...
  it.effect("uses exact static workflow model ids when the provider recognizes them", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runLanguage({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("gitlab"), Model.ID.make("duo-workflow-exact")),
          api: { id: Model.ID.make("duo-workflow-exact"), type: "aisdk", package: "test-provider" },
        }),
        sdk: {
          workflowChat: (id: string, options: unknown) => {
            calls.push([id, options])
