...
describe("PerplexityPlugin", () => {
    ...
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("perplexity"), Model.ID.make("sonar")),
          api: { id: Model.ID.make("sonar"), type: "aisdk", package: "test-provider" },
        }),
  ...
  it.effect("creates bundled Perplexity SDKs for custom provider IDs", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("custom-perplexity"), Model.ID.make("sonar")),
          api: { id: Model.ID.make("sonar"), type: "aisdk", package: "test-provider" },
        }),
        package: "@ai-sdk/perplexity",
  ...
  it.effect("leaves Perplexity language selection to the default languageModel fallback", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runLanguage({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("perplexity"), Model.ID.make("alias")),
          api: { id: Model.ID.make("sonar"), type: "aisdk", package: "test-provider" },
        }),
