...
describe("VenicePlugin", () => {
    ...
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("venice"), Model.ID.make("model")),
  ...
  it.effect("uses the model provider ID as the bundled Venice SDK name", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("custom-venice"), Model.ID.make("model")),
          api: { id: Model.ID.make("model"), type: "aisdk", package: "test-provider" },
        }),
        package: "venice-ai-sdk-provider",
  ...
  it.effect("leaves Venice language selection to the default languageModel fallback", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runLanguage({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("venice"), Model.ID.make("alias")),
          api: { id: Model.ID.make("alias"), type: "aisdk", package: "test-provider" },
        }),
