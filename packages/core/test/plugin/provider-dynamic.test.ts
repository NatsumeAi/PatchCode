...
describe("DynamicProviderPlugin", () => {
    ...
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("custom"), Model.ID.make("test-model")),
          api: { id: Model.ID.make("test-model"), type: "aisdk", package: fixtureProvider },
        }),
  ...
  it.effect("injects the provider ID as the SDK factory name", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("custom-provider"), Model.ID.make("test-model")),
          api: { id: Model.ID.make("test-model"), type: "aisdk", package: fixtureProvider },
        }),
        package: fixtureProvider,
  ...
  it.effect("loads npm packages through their resolved import entrypoint", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runSDK({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("npm-provider"), Model.ID.make("test-model")),
          api: { id: Model.ID.make("test-model"), type: "aisdk", package: "fixture-provider" },
        }),
  ...
  itWithAISDK.effect("wraps missing npm entrypoint failures as AISDK init errors", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* aisdk
        .language(
          Model.Info.make({
            ...Model.Info.empty(Provider.ID.make("missing-entrypoint"), Model.ID.make("alias")),
            api: { id: Model.ID.make("alias"), type: "aisdk", package: "fixture-provider" },
          }),
        )
        .pipe(Effect.exit)
  ...
  itWithAISDK.effect("wraps dynamic import failures as AISDK init errors", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* aisdk
        .language(
          Model.Info.make({
            ...Model.Info.empty(Provider.ID.make("bad-import"), Model.ID.make("alias")),
            api: { id: Model.ID.make("alias"), type: "aisdk", package: "file:///missing/provider-factory.js" },
          }),
        )
        .pipe(Effect.exit)
      expect(exit._tag).toBe("Failure")
  ...
  itWithAISDK.live("wraps missing provider factory exports as AISDK init errors", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* aisdk
        .language(
          Model.Info.make({
            ...Model.Info.empty(Provider.ID.make("missing-factory"), Model.ID.make("alias")),
            api: { id: Model.ID.make("alias"), type: "aisdk", package: "fixture-provider" },
          }),
        )
        .pipe(Effect.exit)
  ...
  itWithAISDK.effect("uses the model api.id for the default language model", () =>
    Effect.gen(function* () {
      ...
      const language = yield* aisdk.language(
        Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("custom"), Model.ID.make("alias")),
