      ...
      Effect.gen(function* () {
        ...
        const sdk = yield* aisdk.runSDK({
          model: Model.Info.make({
            ...Model.Info.empty(Provider.ID.make("cloudflare-workers-ai"), Model.ID.make("@cf/model")),
            api: { id: Model.ID.make("@cf/model"), ...provider.api },
          }),
  ...
  it.effect("preserves a configured endpoint URL instead of deriving one from account ID", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct" }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(Provider.ID.make("cloudflare-workers-ai"))).api).toEqual({
          type: "aisdk",
          package: "test-provider",
  ...
  it.effect("selects languageModel with the API model ID", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runLanguage({
        model: Model.Info.make({
          ...Model.Info.empty(Provider.ID.make("cloudflare-workers-ai"), Model.ID.make("alias")),
          api: { id: Model.ID.make("@cf/api-model"), type: "aisdk", package: "test-provider" },
        }),
