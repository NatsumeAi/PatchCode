import { DialogBody, DialogHeader, DialogTitle, Dialog } from "@opencode-ai/ui/kit/dialog"
        ...
        <div ref={listEl} class="flex min-h-0 flex-col">
          <div data-section="free-models" class="flex w-full flex-col items-start pb-3">
            <div class="flex h-8 w-full flex-none select-none flex-row items-center px-3 pb-2">
              <div class="flex h-5 items-center text-[13px] font-[440] leading-5 tracking-[-0.04px] text-kit-text-text-muted [font-family:var(--kit-font-family-sans)] [font-variant-numeric:tabular-nums] [font-variation-settings:'slnt'_0]">
            ...
            <For each={freeModels()}>
              {(item) => (
                <Tooltip
                  ...
                  gutter={6}
                  openDelay={0}
                  contentStyle={{ "font-family": "var(--kit-font-family-sans)" }}
                  value={
                    <ModelTooltip
                ...
                >
                  <button
                    type="button"
                    class="flex w-full scroll-my-3.5 flex-row items-center gap-1.5 rounded-md px-3 py-2 text-left text-[13px] font-[530] leading-5 tracking-[-0.04px] text-kit-text-text-base [font-family:var(--kit-font-family-sans)] [font-variation-settings:'slnt'_0] hover:bg-kit-overlay-simple-overlay-hover focus:bg-kit-overlay-simple-overlay-hover focus:outline-none"
                  ...
                  >
                    ...
                    <Show when={currentKey() === modelKey(item)}>
                      <Icon name="check" class="ml-auto size-4 shrink-0 text-kit-icon-icon-base" />
                    </Show>
                  </button>
                </Tooltip>
              )}
          ...
          <div class="flex w-full flex-col">
            <div class="flex w-full flex-col items-start rounded-lg border-[0.5px] border-kit-border-border-muted bg-kit-background-bg-layer-02 p-2.5 pt-2">
              ...
              <div class="grid w-full grid-cols-1 gap-y-1.5 gap-x-2 sm:grid-cols-2">
                ...
                >
                  {(provider) => (
                    <button
                      ...
                      class="flex min-h-11 w-full scroll-my-3.5 flex-row items-start gap-2 rounded-md bg-kit-background-bg-base px-3 py-2.5 text-left text-[13px] font-[530] leading-5 tracking-[-0.04px] text-kit-text-text-base [font-family:var(--kit-font-family-sans)] [font-variation-settings:'slnt'_0] hover:bg-kit-background-bg-layer-01 focus:bg-kit-background-bg-layer-01 focus:outline-none"
                      classList={{
                        "border-[0.5px] border-transparent shadow-[var(--kit-elevation-raised)]":
                          theme.mode() !== "dark",
                        "border-[0.5px] border-kit-border-border-strong": theme.mode() === "dark",
                      }}
                      onClick={() => openProviders(provider.id)}
                    >
                      <ProviderIcon id={provider.id} class="mt-0.5 size-4 shrink-0 text-kit-icon-icon-base" />
                      <span class="flex min-w-0 flex-col">
                        <span class="truncate">{provider.name}</span>
                        <Show when={provider.id === "opencode" || provider.id === "opencode-go"}>
                          <span class="truncate font-[440] text-kit-text-text-muted">
                            {language.t(
                ...
                <button
                  type="button"
                  class="col-span-full flex h-8 w-full scroll-my-3.5 items-center justify-start rounded-md px-3 text-left text-[13px] font-[440] leading-5 tracking-[-0.04px] text-kit-text-text-muted [font-family:var(--kit-font-family-sans)] [font-variation-settings:'slnt'_0] hover:bg-kit-overlay-simple-overlay-hover focus:bg-kit-overlay-simple-overlay-hover focus:outline-none"
