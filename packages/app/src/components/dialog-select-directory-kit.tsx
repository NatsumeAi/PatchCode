...
import "./dialog-select-directory-kit.css"
...
export function DialogSelectDirectory(props: DialogSelectDirectoryProps) {
  ...
  onMount(() => {
    ...
    tree = new FileTree({
      ...
      unsafeCSS: `
        ...
        button[data-type="item"]:hover {
          background: var(--kit-overlay-simple-overlay-hover) !important;
        }
    ...
    tree.getFileTreeContainer()?.classList.add("directory-picker-kit-tree")
  })
  ...
  return (
    <Dialog size="large" class="directory-picker-kit">
      <DialogHeader>
        <DialogTitle>{props.title ?? language.t("command.project.open")}</DialogTitle>
      </DialogHeader>
      <Divider />
      <DialogBody class="directory-picker-kit-body pt-4!">
        <div class="directory-picker-kit-path" ref={pathArea}>
          <TextInput
            value={input()}
            autofocus
            autocomplete="off"
            ...
            aria-controls="directory-picker-kit-suggestions"
            aria-activedescendant={
              activeSuggestion() >= 0 ? `directory-picker-kit-suggestion-${activeSuggestion()}` : undefined
            }
            onKeyDown={handleInputKey}
          />
          <div class="directory-picker-kit-actions">
          ...
          <Show when={suggestionsOpen() && currentSuggestions().length > 0}>
            <div id="directory-picker-kit-suggestions" role="listbox" class="directory-picker-kit-suggestions">
              <For each={currentSuggestions()}>
                {(suggestion, index) => (
                  <button
                    id={`directory-picker-kit-suggestion-${index()}`}
        ...
        <div
          class="directory-picker-kit-browser"
          ref={container}
          onWheel={(event) => {
            const scroller = tree
        ...
        >
          <Show when={loading()}>
            <div class="directory-picker-kit-state">{language.t("common.loading")}</div>
          </Show>
          <Show when={!loading() && error()}>
            <div class="directory-picker-kit-state">{language.t("dialog.directory.readError")}</div>
          </Show>
        </div>
        <div class="directory-picker-kit-selection">{policy.result(root(), selected(), rootValid())}</div>
      </DialogBody>
      <DialogFooter>
