...
import { buildFileTreeModel, flattenFileTree, flattenLiveFileTree } from "./file-tree-kit-model"
...
describe("buildFileTreeModel", () => {
  test("builds a sorted tree and flattens expanded directories", () => {
    ...
    expect(flattenFileTree(model, () => true).map((row) => [row.node.path, row.node.type, row.level])).toEqual([
      ["docs", "directory", 0],
  ...
  test("skips children of collapsed directories", () => {
    ...
    expect(flattenFileTree(model, (path) => path !== "src/lib").map((row) => row.node.path)).toEqual([
      "src",
      "src/lib",
      "src/z.ts",
    ])
  ...
  test("normalizes duplicate and messy paths", () => {
    ...
    const rows = flattenFileTree(model, () => true)
  ...
  test("handles deeply nested paths", () => {
    ...
    const model = buildFileTreeModel([file])

    expect(flattenFileTree(model, () => true)).toHaveLength(131)
  })
})

describe("flattenLiveFileTree", () => {
