...
import { Dialog, DialogBody } from "@opencode-ai/ui/kit/dialog"
...
import "./dialog-command-palette-kit.css"
...
function CommandPaletteView(props: {
  ...
  return (
    <Dialog class="command-palette-kit" size="large">
      <DialogBody class="command-palette-kit-body">
        <div class="command-palette-kit-search">
          <TextInput
            value={query()}
            autofocus
            autocomplete="off"
        ...
        <ScrollView class="command-palette-kit-scroll" viewportRef={(el) => (resultsRef = el)}>
          <div class="command-palette-kit-results" role="listbox">
            <Show
              when={visibleEntries().length > 0}
              fallback={
                <div class="command-palette-kit-state">
            ...
            >
              <For each={groupedEntries()}>
                {(group) => (
                  <div class="command-palette-kit-group">
                    <Show when={group.category}>
                      <div class="command-palette-kit-group-title">{group.category}</div>
                    </Show>
...
function PaletteRow(props: {
  ...
  return (
    <button
      type="button"
      class="command-palette-kit-row group"
      role="option"
    ...
    >
      <Switch
        fallback={
          <div class="command-palette-kit-row-main">
            <FileIcon node={{ path: props.item.path ?? "", type: "file" }} class="command-palette-kit-row-icon size-4" />
            <div class="command-palette-kit-file-path">
              <span class="command-palette-kit-file-dir">{getDirectory(props.item.path ?? "")}</span>
              <span class="command-palette-kit-file-name">{getFilename(props.item.path ?? "")}</span>
            </div>
          </div>
        }
      >
        <Match when={props.item.type === "command"}>
          <div class="command-palette-kit-row-main">
            <div class="command-palette-kit-row-text">
              <span class="command-palette-kit-title">{props.item.title}</span>
              <Show when={props.item.description}>
                <span class="command-palette-kit-description">{props.item.description}</span>
              </Show>
            </div>
          </div>
        ...
        <Match when={props.item.type === "session"}>
          <div class="command-palette-kit-row-main">
            <div class="relative shrink-0">
              <Show when={props.sessionOpen}>
                <span
                  aria-hidden="true"
                  class="pointer-events-none absolute top-1/2 h-3 w-0.5 -translate-y-1/2 rounded-[2px] bg-kit-background-bg-layer-04"
            ...
            <div class="command-palette-kit-row-text">
              <span class="command-palette-kit-title" classList={{ "opacity-70": !!props.item.archived }}>
                {props.item.title}
              </span>
              <Show when={props.item.description}>
                <span class="command-palette-kit-description" classList={{ "opacity-70": !!props.item.archived }}>
          ...
          <Show when={props.item.updated}>
            <span class="command-palette-kit-meta">
