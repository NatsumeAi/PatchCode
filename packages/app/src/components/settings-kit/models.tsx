...
export const SettingsModels: Component = () => {
  ...
  const [store, setStore] = persisted(
    Persist.serverGlobal(serverSdk().scope, "settings.models.providers", ["settings-v2.models.providers"]),
  ...
  return (
    <>
      <div class="settings-kit-tab-header settings-kit-tab-header--stacked">
        <h2 class="settings-kit-tab-title">{language.t("settings.models.title")}</h2>
      ...
      <div class="settings-kit-tab-body settings-kit-models">
        <Show
          ...
          >
            <For each={list.grouped.latest}>
              {(group) => {
                ...
                return (
                  <div
                    class="settings-kit-section"
                    data-component="settings-models-provider"
                    data-expanded={expanded() ? "" : undefined}
                  >
                    <h3 class="settings-kit-models-group-header">
                      <button
                        type="button"
                        class="settings-kit-models-group-trigger"
                        aria-expanded={expanded()}
                        disabled={searching()}
                        onClick={() => setStore("collapsed", group.category, expanded())}
                      >
                        <span class="settings-kit-models-group-chevron">
                          <Show
                            when={expanded()}
                            fallback={
                        ...
                        <span class="settings-kit-models-group-label">
                          <ProviderIcon
                            id={group.category}
                            width={PROVIDER_ICON_SIZE}
                            height={PROVIDER_ICON_SIZE}
                            class="settings-kit-models-provider-icon shrink-0"
                          />
                          <span class="settings-kit-section-title">{group.items[0].provider.name}</span>
                        </span>
                      </button>
                    </h3>
                    <Show when={expanded()}>
