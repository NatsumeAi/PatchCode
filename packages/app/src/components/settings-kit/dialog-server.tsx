...
export const DialogServer: Component<{
  ...
  return (
    <Dialog fit class="settings-kit-server-dialog">
      ...
      <DialogBody class="flex w-full min-w-0 flex-1 flex-col px-4 pt-4 pb-2">
        <div class="flex w-full min-w-0 flex-col gap-6">
          <div class="flex w-full min-w-0 flex-col gap-2">
            <label class="settings-kit-server-dialog-label">{language.t("dialog.server.add.url")}</label>
            <TextInput
              type="text"
              appearance="large"
            ...
            <Show when={controller.formError()}>
              <span class="settings-kit-server-dialog-error">{controller.formError()}</span>
            </Show>
          </div>
          <div class="flex w-full min-w-0 flex-col gap-2">
            <label class="settings-kit-server-dialog-label">{language.t("dialog.server.add.name")}</label>
            <TextInput
              type="text"
              appearance="large"
              class="!w-full self-stretch"
          ...
          <div class="grid w-full min-w-0 grid-cols-2 gap-4">
            <div class="flex min-w-0 flex-col gap-2">
              <label class="settings-kit-server-dialog-label">{language.t("dialog.server.add.username")}</label>
              <TextInput
                type="text"
                appearance="large"
                class="!w-full self-stretch"
            ...
            <div class="flex min-w-0 flex-col gap-2">
              <label class="settings-kit-server-dialog-label">{language.t("dialog.server.add.password")}</label>
              <TextInput
                type="password"
                appearance="large"
                class="!w-full self-stretch"
      ...
      <DialogFooter>
        <Button variant="neutral" disabled={controller.formBusy()} onClick={() => dialog.close()}>
          {language.t("common.cancel")}
        </Button>
        <Button variant="contrast" disabled={controller.formBusy()} onClick={controller.submitForm}>
          {submitLabel()}
        </Button>
      </DialogFooter>
    </Dialog>
