...
import { SplitButton, SplitButtonAction, SplitButtonMenuTrigger } from "@opencode-ai/ui/kit/split-button"
...
export function OpenInApp(props: { directory: () => string }) {
  const language = useLanguage()
  const state = useOpenInApp(props)

  return (
    <Show when={props.directory() && state.canOpen()}>
      <SplitButton class="session-review-kit-open-in-app" onPointerDown={(event) => event.stopPropagation()}>
        <Tooltip
          placement="bottom"
        ...
        >
          <Menu.Trigger
            ...
            onPointerDown={(event) => event.stopPropagation()}
          >
            <KitIcon name="chevron-down" size="small" />
          </Menu.Trigger>
          <Menu.Portal>
            <Menu.Content class="open-in-app-kit-menu">
              <Menu.Group>
