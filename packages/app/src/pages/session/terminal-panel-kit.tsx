...
import { SortableTerminalTab } from "@/components/session/session-sortable-terminal-tab-kit"
...
export function TerminalPanel(props: { stacked?: boolean } = {}) {
  ...
  return (
    <aside
      ...
      aria-hidden={!opened()}
      inert={!opened()}
      class="relative shrink-0 overflow-hidden bg-kit-background-bg-base"
      classList={{
        "w-full": !isDesktop() || stacked(),
        "min-w-0 h-full flex-1": isDesktop() && opened() && !stacked(),
        "w-0 h-full pointer-events-none": isDesktop() && !opened(),
        "rounded-[10px] shadow-[var(--kit-elevation-raised)]": isDesktop() && newLayout(),
        ...
        <Show
          ...
          fallback={
            <div class="flex flex-col h-full pointer-events-none">
              <div class="h-10 flex items-center gap-2 px-2 border-b border-border-weaker-base bg-kit-background-bg-base overflow-hidden">
                <For each={handoff()}>
                  {(title) => (
